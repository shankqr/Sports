...
- (void)getNewMessages
{
    [[Globals i] updateMembersData];
    self.messages = [[Globals i] getMembersData];
...
- (UITableViewCell *)tableView:(UITableView *)myTableView cellForRowAtIndexPath:(NSIndexPath *)indexPath 
{
    ...
    int xp = [rowData[@"xp"] intValue];
    int level = [[Globals i] levelFromXp:xp];
    cell.score.text = [NSString stringWithFormat:@"%d", level];
...
{
	NSDictionary *rowData = [[Globals i] getMembersData][indexPath.row];
    ...
    if(![rowData[@"club_id"] isEqualToString:[[Globals i] getClubData][@"club_id"]])
    {
        ...
        {
            selected_aid = [[NSString alloc] initWithString: [[[Globals i] getClubData][@"alliance_id"] stringByReplacingOccurrencesOfString:@"," withString:@""]];
            ...
            UIAlertView *alert = [[UIAlertView alloc]
                                  ...
                                  otherButtonTitles:@"Club Info", @"Kick Out", nil];
            [alert show];
        }
        else
        {
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    if(buttonIndex == 2)
	{
        ...
        NSString *wsurl = [[NSString alloc] initWithFormat:@"%@/AllianceKick/%@/%@/%@",
        ...
        if([returnValue isEqualToString:@"0"])
        {
            ...
            dialogBox.titleText = @"Unable to Kick!";
        ...
        else
        {
            ...
            dialogBox.whiteText = @"Club Kicked Out!";
            dialogBox.promptText = @"The Club will be informed in the News that they have been Kicked Out.";
            dialogBox.dialogType = 1;
            [self.view insertSubview:dialogBox.view atIndex:17];
            [dialogBox updateView];
        }
