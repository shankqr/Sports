...
- (IBAction)sendClicked:(id)sender
{
    ...
    {
		if([[[Globals i] doChat:messageText.text] isEqualToString:@"1"])
		{
            [self getNewMessages];
        }
	}
...
- (void)getNewMessages
{
    self.messages = [[Globals i] wsChatFullData];
...
{
	NSDictionary *rowData = [[Globals i] wsChatFullData][indexPath.row];
