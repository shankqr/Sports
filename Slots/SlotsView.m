...
{
    int diamonds_balance = [Globals.i.wsClubDict[@"currency_second"] intValue];
    ...
    for (NSDictionary *temp in combinations)
    {
        ...
        for (NSString *str in thisCombination)
        {
            UIImageView *image = [[UIImageView alloc] initWithImage:
                                  [UIImage imageNamed:[NSString stringWithFormat:@"%@", str]]];
...
- (void)initializeReels
{
    ...
    for (int i = 0; i < [reels count]; i++)
    {
        ...
        for (int j = 0; j < [[reels objectAtIndex:i] count] + 2; j++)
        {
            ...
            [currentImage setImage:[UIImage imageNamed:[NSString stringWithFormat:@"%@", currentItem]]];
    ...
    [Globals getServer:wsurl :^(BOOL success, NSData *data)
     {
         if (success)
         {
             ...
             {
                 int diamonds_balance = [Globals.i.wsClubDict[@"currency_second"] intValue] + [gameMechanics getCoinsUsed]*1;
                 
                 Globals.i.wsClubDict[@"currency_second"] = [@(diamonds_balance) stringValue];
             }
...
- (void)setFutureImage:(int)reel
{
    ...
    if (final_type == 1)
    {
        [currentImage setImage:[UIImage imageNamed:@"seven"]];
    }
    else if (final_type == 2)
    {
        [currentImage setImage:[UIImage imageNamed:@"bell"]];
    }
    else if (final_type == 3)
    {
        [currentImage setImage:[UIImage imageNamed:@"grapes"]];
    }
    else if (final_type == 4)
    {
        [currentImage setImage:[UIImage imageNamed:@"lemon"]];
    }
    else if (final_type == 5)
    {
        [currentImage setImage:[UIImage imageNamed:@"luck"]];
    }
