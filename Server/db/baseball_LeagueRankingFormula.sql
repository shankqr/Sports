SELECT club_id FROM (select CAST((CAST(LDLeader.Win as smallint)-View_Series.Win-LDLeader.Lose+View_Series.Lose )/2.0 as decimal(4,1)) as Pts,
...
SELECT club_id FROM View_Series WHERE division=1 AND series=1 ORDER BY GA DESC