...
- (id)init
{
    if (self = [super init])
    {
        ...
        {
            myImage = [SPImage imageWithContentsOfFile:@"engine_ball@2x.png"];
            myImage.scaleX = 2.0f;
            myImage.scaleY = 2.0f;
        }
        else
        {
            myImage = [SPImage imageWithContentsOfFile:@"engine_ball.png"];
        }
...
- (void)move
{
    ...
    if((fabs(self.vector.x) < .1) && (fabs(self.vector.y) < .1))
    {
...
- (SPPoint *)moveTowards:(SPPoint *)obj
{
    ...
    double distX = finalX - self.xx;
    double distY = finalY - self.yy;
...
- (void)moveTo:(double)mX mY:(double)mY
{
    ...
    self.y = mY + PITCH_HEIGHT_OFFSET + 15 - (self.offGround); //.7 foreshortening of the view
...
- (void)debug
{
    NSLog(@"Ball atPlayer:%ld kickedByPlayer:%ld movingToObj:%ld steps:%ld C(%.2f , %.2f) V<%.2f , %.2f>", (long)player.player_id, (long)kickedBy.player_id, (long)movingToObj.player_id, (long)steps, xx, yy, vector.x, vector.y);
}
