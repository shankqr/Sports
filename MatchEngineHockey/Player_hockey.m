...
- (id)init:(NSInteger)p_id number:(NSInteger)num jersey:(NSInteger)jer team:(NSInteger)t role:(NSInteger)r name:(NSString*)n
{
    if (self = [super init]) 
    {
        ...
        if (self.team == T_AWAY) 
        {
            ...
            myTextField.fontSize = 38.0*PLAYER_SCALE;
            myTextField.color = 0xEE7600;
...
- (NSInteger)pass:(double)urgency
{
    ...
    for (Player_hockey *p in (GameController.teams)[team]) 
    {
        ...
        if (p.player_id != self.player_id) 
        {
            ...
            Player_hockey *mark = [p getNearest:NO];
    ...
    if ((self.role==P_GOALKEEPER) && (distToNearest<[self getDistance:[SPPoint pointWithX:obj.xx y:obj.yy]]))
    {
        ...
        for (Player_hockey *p in (GameController.teams)[self.team])
        {
...
- (void)passTo:(Player_hockey *)obj
{
    ...
    if (dist > PITCH_WIDTH/4) 
    {
        ...
        ball.speedV = dist * GRAVITY / (2.5*ball.speed); //2*
    }
    else
    {
...
- (void)returnTo:(double)pitchProportion pitchRandMultiplier:(double)pitchRandMultiplier narrowness:(double)narrowness
{
    ...
    Goal_hockey *moveToGoal = self.defendingGoal;
    if (teamHasBall)
    {
...
- (void)turn
{
    ...
    else if (self.actionCountdown == 1)
    {
        ...
        if (self.action == ACTION_MOVINGTHROW) 
        {
            [ball reset];
            [self gotBall];
            Player_hockey *nearest = [self getNearest:YES];
            self.action = ACTION_IDLE;
            [self passTo:nearest];
    ...
    else if ((playerHasBall && ([[Globals i] Random_next:0 to:1]<urgencyToPass/30))
    ...
    else if(self.action == ACTION_GETTINGBALL)
    {
        ...
        else if(distToBall > PITCH_WIDTH/10)
        {
            //[self decideMove];
            self.action = ACTION_PASSEDBALL;
        }
        else
        {
            //Tackle?
